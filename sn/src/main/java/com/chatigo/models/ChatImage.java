...
public class ChatImage {
	...
	public ChatImage(MultipartFile file, int chatId) {
		setFile(file);
		setChatId(chatId);
		if(validate == ValidateStatus.OK) {
			try {
				
				Files.createDirectories(Paths.get(dbconnection.getPhotoPath() + "chats\\" + chatId));
				file.transferTo(new File(dbconnection.getPhotoPath() + "chats\\" + chatId + "\\" +  file.getOriginalFilename()));
				path = dbconnection.getPhotoPath() + "chats\\" + chatId + "\\" +  file.getOriginalFilename();
				
				dbconnection.addImageToImage(path);
				dbconnection.addChatImage(chatId, path);
	...
	public void setChatId(int chatId) {
		if(chatId != 0 && dbconnection.checkChatById(chatId)) {
			this.chatId = chatId;
		}
		else {
